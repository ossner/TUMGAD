\textbf{\LARGE{\color{tumgadPurple}Graph Traversal (DFS \& BFS)}}\\
\\
\noindent
Given the following directed graph:
\begin{center}
    \begin{tikzpicture}[
        > = stealth, % arrow head style
        auto,
        node distance = 2.25cm, % distance between nodes
        semithick % line style
    ]
        \node[state] (00) {$0$};
        \node[state, draw=none] (01) [right of=00] {%$01$
        };
        \node[state, draw=none] (02) [right of=01] {%$02$
        };
        \node[state, draw=none] (03) [right of=02] {%$03$
        };

        \node[state, draw=none] (10) [below of=00] {%$10$
        };
        \node[state, draw=none] (11) [right of=10] {%$11$
        };
        \node[state, draw=none] (12) [right of=11] {%$12$
        };
        \node[state, draw=none] (13) [right of=12] {%$13$
        };

        \node[state, draw=none] (20) [below of=10] {%$20$
        };
        \node[state, draw=none] (21) [right of=20] {%$21$
        };
        \node[state, draw=none] (22) [right of=21] {%$22$
        };
        \node[state, draw=none] (23) [right of=22] {%$23$
        };

        \node[state, draw=none] (30) [below of=20] {%$30$
        };
        \node[state, draw=none] (31) [right of=30] {%$31$
        };
        \node[state, draw=none] (32) [right of=31] {%$32$
        };
        \node[state, draw=none] (33) [right of=32] {%$33$
        };

        %$CONNECTIONS$
    \end{tikzpicture}
\end{center}
a) In which order will the nodes in the graph be visited when traversing
the graph with \href{https://ossner.github.io/TUMGAD/src/Algorithms/Graphs/Traversal/BFS}{\underline{Breadth-First Search}}?\\
Start at node 0.\\
\\
\noindent\fbox{
    \parbox{\textwidth}{
    \vspace{0.6cm}
    \hspace{\textwidth}
    }
}\\
\vspace{10px}
\\
b) In which order will the nodes in the graph be visited when traversing
the graph with \href{https://ossner.github.io/TUMGAD/src/Algorithms/Graphs/Traversal/DFS}{\underline{Depth-First Search}}?\\
Start at node 0 again.\\
\\
\noindent\fbox{
    \parbox{\textwidth}{
        \vspace{0.6cm}
        \hspace{\textwidth}
    }
}
